...
If you want a side-by-side install that does not replace the Homebrew app, use the custom build and put `AeroSpace Custom.app` to `/Applications` instead.
The custom app has its own bundle identifier, Accessibility permission, and socket path, so it can coexist with the normal `AeroSpace.app`.
The custom build also installs a separate `aerospace-custom` CLI binary, and the normal `aerospace` command auto-routes to `AeroSpace Custom.app` when it is running.
